...
public class TUIModuleFactory {
    ...
    public static TUIContainerModule.Builder NumberedList(String name, String... text) {
        return NumberedList(name, 1, 1, text);
    }

    public static TUIContainerModule.Builder NumberedList(String name, int start, int step, String... text) {
        TUIContainerModule.Builder list = new TUIContainerModule.Builder(name);

        for(int i = 0; i < text.length; i ++) {
            list.addChild(
                    new TUITextModule.Builder(name + "-" + ((i + start) * step), ((i + start) * step) + ". " + text[i]).build()
            );
        }
    ...
    public static TUIContainerModule.Builder NumberedListInput(String name, String inputMessage, String... text) {
        return new TUIContainerModule.Builder(name)
                .children(
                        NumberedList(name + "-list", text).build(),
                        new TUITextInputModule.Builder(name + "-input", inputMessage).build()
                );
    }

    public static TUIContainerModule.Builder NumberedModuleSelector(String name, List<String> displayText, TUIApplicationModule app, List<String> moduleNames) {
        return new TUIContainerModule.Builder(name)
                .children(
                        NumberedListInput(name+"-input-list", "Your input: ", displayText.toArray(new String[0])).build(),
                        new TUIFunctionModule.Builder(name+"-get-module-function", () -> {
                            int choice;
                            TUIModule module;
                            try {
                                String choiceInput = app.getInput(name + "-input-list-input", String.class);
                                if(choiceInput == null) return "Error: the app didn't properly save the input";
                                choice = Integer.parseInt(choiceInput);
                                module = app.getChild(moduleNames.get(choice - 1));
                                if(module == null) return "Error: The selected scene doesn't exist";
                            }
                            catch(NumberFormatException|IndexOutOfBoundsException e) {
                                System.out.println("Error: Invalid Input (1-" + moduleNames.size() + ")");
                                TUIModule thisModule = app.getChild(name);
                                if(thisModule == null) return "Error: this module didn't get added to the app";
                                thisModule.run();
                                return "Error: Invalid User Input";
                            }
                            module.run();
                            return "Success: retrieved scene" + module.getName();
                        }).build()
                );
    }

    public static TUIContainerModule.Builder NumberedModuleSelector(String name, TUIApplicationModule app, List<String> displayText, List<TUIModule> modules) {
        return new TUIContainerModule.Builder(name)
                .children(
                        NumberedListInput(name+"-input-list", "Your input: ", displayText.toArray(new String[0])).build(),
                        new TUIFunctionModule.Builder(name+"-get-module-function", () -> {
